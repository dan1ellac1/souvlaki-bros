...
export const NumberConfirmation= ({phoneNumber}) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#f5f5f5]">
      <div className="bg-white p-10 rounded-lg shadow-md text-center w-[400px]">
        <h2 className="text-xl font-bold">Is this your Correct Number? <br/> {phoneNumber}</h2>
        ...
        <button onClick={handlePhoneFalse} className={`w-full border-2 bg-black text-white font-bold py-2 rounded`}>Change phone number</button>
      </div>
    </div>
  )
}
